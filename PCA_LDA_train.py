...
temp_x, waste_x = train_test_split(test_data, test_size=0.2, random_state=42)
test_data = temp_x
...
train_x, test_x, train_y, test_y = train_test_split(transformed_samples_final, y, test_size=0.5, random_state=42)
#lbfgs
...
filename1 = 'PCA_LDA_Tree_model.sav'
...
for lab,marker,color in zip(
        ...
        plt.scatter(x=transformed_samples_final[:,0].real[y == lab],
                y=transformed_samples_final[:,1].real[y == lab],
                marker=marker,
                color=color,
                alpha=0.5,
...
plt.title('Dimensionality reduction by combination of PCA and LDA')
plt.savefig('PCA_LDA.png')                     
plt.show()